...
def repeat(sequence, spectrometer, period_minutes = .05, write_to = 'database', experiment_names=None):
    ...
    if experiment_name==None:
        ...
        for name in experiment_names:
            db_group = db.create_group(db.root, name, 'Sensor: '+name)    
    
    # Create first array in database and fill it with wavelengths [nm]
    db_entry = db.create_array(db.root,'wavelength', spectrometer.WL, 'Wavelength binning provided by '+spectrometer.name)
